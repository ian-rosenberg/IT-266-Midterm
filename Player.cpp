...
bool idInventory::UseAmmo( int index, int amount ) {
	...
	if (owner->mana >= amount ) {
		owner->mana -= amount;//will change per weapon
...
void idPlayer::UpdateHudStats(idUserInterface *_hud) {
	...
	if (healthToAdd && inventory.maxHealth < MAX_STAT){
		...
		{
		case 1: if (enemiesKilled >= 1){
					AddHP();
		}
				break;

		case 2: if (enemiesKilled >= 5){
					AddHP();
		}
				break;

		case 3: if (enemiesKilled >= 10){
					AddHP();
		}
				break;

		case 4: if (enemiesKilled >= 20){
					AddHP();
		}
				break;

		case 5: if (enemiesKilled >= 50){
					AddHP();
		}
				break;
		}
	...
	switch (inventory.level){
		case 1:_hud->HandleNamedEvent("Level1");
			break;
		case 2:_hud->HandleNamedEvent("Level2");
			break;
		case 3:_hud->HandleNamedEvent("Level3");
			break;
		case 4:_hud->HandleNamedEvent("Level4");
			break;
		case 5:_hud->HandleNamedEvent("Level5");
			break;
	}
...
void idPlayer::UpdatePowerUps( void ) {
	...
	if (gameLocal.time >= MANA_PULSE){//add levelModifier
		if (mana < inventory.maxMana){
			mana += 1;
			nextManaPulse = 0;
		}
...
void idPlayer::Weapon_Usable( void ) {
	...
	if ( ( usercmd.buttons & BUTTON_ATTACK ) && !( oldButtons & BUTTON_ATTACK && mana ) ) {
